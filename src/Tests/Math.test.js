 ...
 */
 ...
 const {addUp, subtract, multiply, divide, factorial, toPower, squareRoot, abs, logarithm, nthSquareRoot} = require('../Math');
 ...
 test('addUp_test', () => {
     ...
     expect(addUp(1, -1.234)).toBeCloseTo(-0.234,3); // failing - eps
     expect(addUp(-1, -1.234)).toBe(-2.234);
     ...
     expect(addUp(-1.234, 2.345)).toBeCloseTo(1.111,3); // failing - eps
     expect(addUp(1.234, -2.345)).toBeCloseTo(-1.111,3); // failing - eps
     expect(addUp(-1.234, -1.234)).toBe(-2.468);
 ...
 test('subtract_test', () => {
     ...
     /** Testing basic functionality with two doubles */
     expect(subtract(2.345, 1.234)).toBeCloseTo(1.111,3); // failing - eps
     expect(subtract(1.234, 2.345)).toBeCloseTo(-1.111,3); // failing - eps
 ...
 test('divide_test', () => {
     ...
     expect(divide(1.234, 2.345)).toBeCloseTo(0.526226012793177,14); // failing - eps
 ...
 test('toPower_test', () => {
     ...
     expect(toPower(1.234, 2)).toBeCloseTo(1.522756,6); // failing eps
     expect(toPower(1.234, 3)).toBeCloseTo(1.879080904,9);
     expect(toPower(-1.234, 1)).toBeCloseTo(-1.234,3);
     expect(toPower(-1.234, 2)).toBeCloseTo(1.522756,6);
     expect(toPower(-1.234, 3)).toBeCloseTo(-1.879080904,9);
     
 
     /** Testing exception - zero and less */
     expect(toPower(5, 0)).toBe(1); // not throwing exception
     expect(toPower(5, -1)).toBe(0.2); // not throwing exception
 });
 ...
 test('squareRoot_test', () => {
     ...
     expect(nthSquareRoot(2, 0)).toBe(0);
     expect(nthSquareRoot(2, 1)).toBe(1);
     expect(nthSquareRoot(2, 4)).toBe(2);
     expect(nthSquareRoot(2, 5)).toBe(2.23606797749979);
     
 
     /** Testing basic functionality with double */
     expect(squareRoot(0.555)).toBe(0.744983221287567);
     expect(nthSquareRoot(2, 1.234)).toBe(1.1108555261599053);
 
 
     /** Testing extended functionality with integer */
     expect(nthSquareRoot(3, 1)).toBe(1);
     expect(nthSquareRoot(3, 8)).toBe(2); // failing
     expect(nthSquareRoot(3, 2)).toBeCloseTo(1.25992105,8);
 
 
     /** Testing extended functionality with double */
     expect(nthSquareRoot(3,1.234)).toBeCloseTo(1.07260147,8);
 
     /** Testing exception - less than zero */
     expect(() => nthSquareRoot(2,-1)).toThrow();
     expect(nthSquareRoot(-1,2)).toBe(0.5);
 
     /**Testin basic squareRoot */
 ...
 test('logarithm_test', () => {
     ...
     /** Testing exception - zero and less */
     //expect(() => logarithm(0)).toThrow(); // not throwing exception - does not need to throw = - inf
     expect(() => logarithm(-0.555)).toThrow(); 
 });
